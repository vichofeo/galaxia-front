  ...
  <v-container fluid v-if="workitem">
    <v-card>
      ...
      <v-card-title class="primary white--text">
        <v-icon left>{{ getActivityIcon(workitem.activity.type) }}</v-icon>
        {{ workitem.activity.name }}
        <v-spacer></v-spacer>
        <v-chip dark>{{ workitem.activity.type }}</v-chip>
      </v-card-title>
      ...
      <v-card-text class="pa-6">
        <!-- Descripción -->
        <div class="mb-6" v-if="workitem.activity.description">
          <h4 class="subtitle-1">Descripción:</h4>
          <p class="body-1">{{ workitem.activity.description }}</p>
        </div>

        <!-- Formulario Interactivo -->
        <div v-if="workitem.activity.isInteractive && !activityCompleted">
          <v-form ref="activityForm" v-model="formValid">
            ...
            <div class="mb-4">
              ...
              <v-chip v-for="(value, key) in workitem.instance.data" :key="key" class="ma-1" small>
                {{ key }}: {{ value }}
              </v-chip>
            </div>
          </v-form>
        ...
        <div v-else-if="!workitem.activity.isInteractive && !activityCompleted">
...
export default {
  ...
  methods: {
    ...
    async completeActivity() {
      this.executing = true
      try {
        await srv.saveCompleteWorkItem(this.workitem.id, this.formData)
